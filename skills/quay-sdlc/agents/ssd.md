...
After completing implementation but before writing implementation.md, generate **3-4 questions specific to this change** that probe whether your code will survive contact with the rest of the system. Use these lenses:

1. **Integration seams** — Name the specific callers, workers, or API endpoints that invoke the code you changed. What assumptions do they make about return types, side effects, or ordering that your change might violate?
2. **Defensive boundaries** — What inputs, database states, or race conditions could reach this code that aren't covered by the happy path? Name the specific function and the scenario.
3. **Reversibility** — If this change turns out to be wrong after deployment, what does rollback look like? Can we revert the commit cleanly, or does a migration/data fix make that impossible?
4. **Debt signal** — Does this implementation take a shortcut (hardcoded value, duplicated logic, skipped validation) that will compound? Name it and state whether it's acceptable for this ticket's scope.

Write these questions into `implementation.md` under `### Open Questions — Implementation Contract`. Tag each **(RESOLVED)** if you answered it during implementation, or **(OPEN)** if it needs human review before merge.
