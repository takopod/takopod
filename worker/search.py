...
# ---------------------------------------------------------------------------
# Legacy functions (no longer called, kept for backward compat)
# ---------------------------------------------------------------------------
...
def index_message(
    ...
    """Index a message into FTS5. DEPRECATED — no longer called."""
    now = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    conn.execute(
...
async def index_vector(
    ...
    """Index a message into vec0. DEPRECATED — no longer called."""
    try:
